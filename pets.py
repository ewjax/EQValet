...
# Has information about this particular pet spell, as well as an array of PetStats info records, for each possible pet level
#
class PetSpell:
...
class Pet:
    ...
    def __repr__(self):
        rv = 'Pet: {}, Level: {}, Max Melee: {}, Rank (1-{}): {}'.format(self.pet_name, self.pet_level, self.max_melee, len(self.pet_spell.pet_stats_list), self.pet_rank)
        if self.pet_spell:
            rv += ' ({})'.format(self.pet_spell.spell_name)
        return rv
...
class PetTracker:
    ...
    async def process_line(self, line):
        ...
        #
        # if the flag is set that we have a lifetap message and don't have the amount yet, search for the non-melee message
        #
